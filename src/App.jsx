...
export default function CRM() {
  ...
  const [boardPosts, setBoardPosts] = useState([
    ...
    { id: 4, author: "James Reyes", avatar: "JR", category: "Concern", title: "Internet is slow in the afternoon", body: "Not sure if it's just me but the connection gets really sluggish after 2pm. Affects my calls with clients. Anyone else experiencing this?", date: "2026-05-14", pinned: false, reactions: { "👍": ["Carlo Lim"], "❤️": [], "🔥": [] }, comments: [{ author: "Admin User", avatar: "AD", text: "Thanks for flagging James, I'll check with the IT team.", date: "2026-05-14" }] },
  ]);
  ...
  const handleClockIn = () => {
    ...
    const todayRec = { date: today, in: now, out: null, status: now > "08:15" ? "Late" : "Present" };
    setAttendance(prev => {
      const existing = prev[currentUser.id] || [];
      const filtered = existing.filter(r => r.date !== today);
      return { ...prev, [currentUser.id]: [...filtered, todayRec] };
    });
  };
  ...
  const handleClockOut = () => {
    ...
    setAttendance(prev => {
      ...
      return { ...prev, [currentUser.id]: recs.map(r => r.date === today ? { ...r, out: now } : r) };
    });
  };
  ...
  const getAiInsight = async () => {
    ...
    const topAgent = agents.reduce((best, a) => performance[a.id]?.revenue > (performance[best.id]?.revenue || 0) ? a : best, agents[0]);
    const prompt = `You are a CRM analyst. Here's the team summary:
- Total revenue this month: ₱${totalRevenue.toLocaleString()}
- Total deals closed: ${totalDeals}
- Top performer: ${topAgent.name} (₱${performance[topAgent.id]?.revenue.toLocaleString()})
- Leads pipeline: ${leads.length} leads, ${leads.filter(l => l.status === "Closed Won").length} won, ${leads.filter(l => l.status === "New").length} new
- Attendance issues today: ${Object.values(attendance).flat().filter(r => r.date === "2026-05-15" && (r.status === "Late" || r.status === "Absent")).length} agents

Give a short (3-4 sentences) performance insight and 2 actionable recommendations for the team manager. Be direct and practical.`;
    try {
  ...
  const s = {
    ...
    sidebar: { width: 220, background: "#161b27", borderRight: "1px solid #1e2535", display: "flex", flexDirection: "column", padding: "0" },
    ...
    navItem: (active) => ({ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 8, marginBottom: 2, cursor: "pointer", fontSize: 13.5, fontWeight: active ? 600 : 400, background: active ? "#1e3a5f" : "transparent", color: active ? "#60a5fa" : "#8899b4", transition: "all 0.15s" }),
    ...
    btn: (variant = "primary") => ({
      ...
      color: variant === "ghost" ? "#8899b4" : "#fff", transition: "opacity 0.15s",
    }),
  ...
  const AdminDashboard = () => (
    <div>
      <div style={s.grid4}>
        {[
          { label: "Total Revenue", val: `₱${totalRevenue.toLocaleString()}`, change: "+12% this month", pos: true, icon: "💰" },
          { label: "Deals Closed", val: totalDeals, change: "+3 this week", pos: true, icon: "🤝" },
          { label: "Active Leads", val: leads.filter(l => l.status !== "Closed Lost").length, change: `${leads.filter(l => l.status === "New").length} new`, pos: true, icon: "🎯" },
          { label: "Team Present", val: `${agents.length - 1}/${agents.length}`, change: "1 absent today", pos: false, icon: "👥" },
        ].map((stat, i) => (
      ...
      <div style={s.card}>
        ...
        {aiInsight ? (
          <div style={{ fontSize: 13, color: "#c8d4e8", lineHeight: 1.8, background: "#0f1117", borderRadius: 10, padding: "14px 18px" }}>{aiInsight}</div>
        ) : (
          <div style={{ fontSize: 13, color: "#4b5a78" }}>Click "Generate Insight" to get an AI-powered analysis of your team's performance.</div>
        )}
      </div>
      <div style={s.card}>
        <div style={{ fontSize: 14, fontWeight: 700, color: "#fff", marginBottom: 14 }}>🔔 Recent Notifications</div>
        {notifications.map(n => (
          <div key={n.id} style={{ display: "flex", gap: 12, padding: "10px 0", borderBottom: "1px solid #0f1117" }}>
            ...
            <div>
              <div style={{ fontSize: 13, color: "#c8d4e8" }}>{n.text}</div>
              <div style={{ fontSize: 11, color: "#4b5a78", marginTop: 2 }}>{n.time}</div>
            </div>
          </div>
        ))}
      </div>
    ...
    <div>
      ...
      <div style={s.card}>
        <table style={s.table}>
          <thead>
            <tr>
              {["Company", "Contact", "Status", "Assigned To", "Value", "Date", "Action"].map(h => (
            ...
            {leads.map(lead => {
              ...
              return (
                <tr key={lead.id} style={{ cursor: "pointer" }} onClick={() => setLeadModal(lead)}>
                  ...
                  <td style={s.td}><span style={{ fontSize: 12 }}>{agent?.name || "Unassigned"}</span></td>
                  <td style={s.td}><span style={{ color: "#10b981", fontWeight: 600 }}>₱{lead.value.toLocaleString()}</span></td>
                  <td style={s.td}>{lead.date}</td>
                  <td style={s.td} onClick={e => { e.stopPropagation(); setLeads(prev => prev.filter(l => l.id !== lead.id)); }}>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 }}>
          {filtered.map((agent, rank) => {
            ...
            const wonLeads = agentLeads.filter(l => l.status === "Closed Won");
            ...
            return (
              <div key={agent.id}
                ...
                style={{ background: "#161b27", borderRadius: 16, border: "1px solid #1e2535", padding: 22, cursor: "pointer", transition: "border-color 0.2s", position: "relative", overflow: "hidden" }}
                onMouseEnter={e => e.currentTarget.style.borderColor = "#3b82f6"}
                onMouseLeave={e => e.currentTarget.style.borderColor = "#1e2535"}
              >
                {/* Rank badge */}
                {rank < 3 && sortBy === "revenue" && (
                  <div style={{ position: "absolute", top: 14, right: 14, fontSize: 20 }}>{rankIcons[rank]}</div>
                )}

                {/* Header */}
                <div style={{ display: "flex", gap: 14, marginBottom: 18 }}>
                  <div style={{ width: 52, height: 52, borderRadius: "50%", background: "linear-gradient(135deg,#3b82f6,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, color: "#fff", flexShrink: 0 }}>{agent.avatar}</div>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                      <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: "#10b98122", color: "#10b981" }}>Active</span>
                      {todayAtt && <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: (todayAtt.status === "Present" ? "#10b981" : todayAtt.status === "Late" ? "#f59e0b" : "#ef4444") + "22", color: todayAtt.status === "Present" ? "#10b981" : todayAtt.status === "Late" ? "#f59e0b" : "#ef4444" }}>Today: {todayAtt.status}</span>}
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 16 }}>
                  {[
                    { label: "Revenue", val: `₱${(perf.revenue || 0).toLocaleString()}`, color: "#10b981" },
                    { label: "Deals", val: perf.deals || 0, color: "#60a5fa" },
                    { label: "Calls", val: perf.calls || 0, color: "#a78bfa" },
                  ].map(m => (
                ...
                <div style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                    <span style={{ fontSize: 11, color: "#4b5a78", fontWeight: 600 }}>TARGET ATTAINMENT</span>
                  ...
                  <div style={{ height: 7, background: "#1e2535", borderRadius: 99, overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${Math.min(attainment, 100)}%`, background: attainment >= 100 ? "#10b981" : attainment >= 70 ? "#f59e0b" : "#ef4444", borderRadius: 99, transition: "width 0.5s" }} />
                  </div>
                  <div style={{ fontSize: 10, color: "#4b5a78", marginTop: 3 }}>{perf.deals} of {agent.target} deals</div>
                </div>

                {/* Attendance Bar */}
                <div style={{ marginBottom: 14 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 5 }}>
                    <span style={{ fontSize: 11, color: "#4b5a78", fontWeight: 600 }}>ATTENDANCE RATE</span>
                ...
                <div style={{ background: "#0f1117", borderRadius: 10, padding: "10px 12px" }}>
                  <div style={{ fontSize: 11, color: "#4b5a78", fontWeight: 600, marginBottom: 8 }}>SALES PIPELINE</div>
                  <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                    {["New", "Follow-up", "Negotiation", "Closed Won"].map(st => {
                      const count = agentLeads.filter(l => l.status === st).length;
                      return count > 0 ? (
                        <span key={st} style={{ display: "inline-flex", alignItems: "center", gap: 4, padding: "3px 8px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: (statusColors[st] || "#4b5a78") + "22", color: statusColors[st] || "#4b5a78" }}>
                          {st} <span style={{ background: statusColors[st], color: "#fff", borderRadius: "50%", width: 14, height: 14, display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 9 }}>{count}</span>
                        </span>
                      ) : null;
                    })}
                    {agentLeads.length === 0 && <span style={{ fontSize: 11, color: "#4b5a78" }}>No leads assigned</span>}
                  </div>
                  <div style={{ marginTop: 6, fontSize: 10, color: "#4b5a78" }}>Total pipeline value: <span style={{ color: "#10b981", fontWeight: 700 }}>₱{agentLeads.reduce((s, l) => s + l.value, 0).toLocaleString()}</span></div>
                </div>

                <div style={{ marginTop: 12, fontSize: 11, color: "#4b5a78", textAlign: "right" }}>Click to view full profile →</div>
              </div>
            );
          })}
        </div>
            ...
            <div style={{ position: "fixed", inset: 0, background: "#000b", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }} onClick={() => setSelectedAgent(null)}>
              <div style={{ background: "#161b27", borderRadius: 20, border: "1px solid #1e2535", padding: 32, width: 560, maxHeight: "88vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ display: "flex", gap: 16, marginBottom: 24, alignItems: "center" }}>
                  <div style={{ width: 64, height: 64, borderRadius: "50%", background: "linear-gradient(135deg,#3b82f6,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, fontWeight: 800, color: "#fff" }}>{ag.avatar}</div>
                  <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10, marginBottom: 20 }}>
                  {[
                    ...
                    { label: "Deals", val: perf.deals || 0, color: "#60a5fa" },
                    { label: "Calls", val: perf.calls || 0, color: "#a78bfa" },
                    { label: "Tasks", val: perf.tasks || 0, color: "#f59e0b" },
                  ].map(m => (
                ...
                <div style={{ background: "#0f1117", borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
                  ...
                  <div style={{ fontSize: 11, color: "#4b5a78", marginTop: 5 }}>{perf.deals} closed of {ag.target} target deals</div>
                </div>
                ...
                <div style={{ background: "#0f1117", borderRadius: 12, padding: "14px 16px", marginBottom: 14 }}>
                  ...
                  {attRecs.map((r, i) => (
                    <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: "1px solid #1e2535" }}>
                      ...
                      <span style={{ display: "inline-block", padding: "2px 10px", borderRadius: 20, fontSize: 11, fontWeight: 600, background: (attColors[r.status] || "#4b5a78") + "22", color: attColors[r.status] || "#4b5a78" }}>{r.status}</span>
                      <span style={{ fontSize: 11, color: "#4b5a78" }}>{r.in ? `${r.in} – ${r.out || "ongoing"}` : "—"}</span>
                    </div>
                  ))}
                  <div style={{ marginTop: 8, fontSize: 12, color: "#4b5a78" }}>Attendance rate: <span style={{ color: "#fff", fontWeight: 700 }}>{attRecs.length ? Math.round((presentDays / attRecs.length) * 100) : 0}%</span></div>
                </div>

                {/* Leads */}
                <div style={{ background: "#0f1117", borderRadius: 12, padding: "14px 16px" }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#fff", marginBottom: 10 }}>Assigned Leads ({agentLeads.length})</div>
                  ...
                  {agentLeads.map(lead => (
                    <div key={lead.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "7px 0", borderBottom: "1px solid #1e2535" }}>
                      ...
                      <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 600, background: (statusColors[lead.status] || "#4b5a78") + "22", color: statusColors[lead.status] || "#4b5a78" }}>{lead.status}</span>
                      <span style={{ fontSize: 12, color: "#10b981", fontWeight: 700 }}>₱{lead.value.toLocaleString()}</span>
                    </div>
                  ))}
                  {agentLeads.length > 0 && <div style={{ marginTop: 8, fontSize: 12, color: "#4b5a78" }}>Total value: <span style={{ color: "#10b981", fontWeight: 700 }}>₱{agentLeads.reduce((s, l) => s + l.value, 0).toLocaleString()}</span></div>}
                </div>
              </div>
            </div>
          );
            ...
            <div key={agent.id} style={s.card}>
              <div style={{ display: "flex", gap: 14, marginBottom: 16 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#4b5a78" }}>{agent.department} · {agent.email}</div>
                  <span style={s.badge("#10b981")}>Active</span>
                </div>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {[
                  { label: "Deals", val: perf.deals || 0 },
                  { label: "Revenue", val: `₱${(perf.revenue || 0).toLocaleString()}` },
                  { label: "Calls", val: perf.calls || 0 },
                  { label: "Attendance", val: `${presentDays}/${attRecs.length}d` },
                ].map(m => (
      ...
      <table style={s.table}>
        <thead>
          <tr>
            ...
            {["May 12", "May 13", "May 14", "May 15"].map(d => <th key={d} style={s.th}>{d}</th>)}
              ...
              <tr key={agent.id}>
                <td style={s.td}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={s.avatar(28)}>{agent.avatar}</div>
                ...
                {recs.map((r, i) => (
                  <td key={i} style={s.td}>
                    ...
                    {r.in && <div style={{ fontSize: 10, color: "#4b5a78", marginTop: 3 }}>{r.in} – {r.out || "—"}</div>}
                  </td>
                ))}
                <td style={s.td}>
                  <div style={{ fontWeight: 700, color: present >= 3 ? "#10b981" : "#f59e0b" }}>{Math.round((present / recs.length) * 100)}%</div>
                </td>
              </tr>
            );
          })}
    ...
    <div>
      <div style={s.card}>
        <table style={s.table}>
          <thead>
            <tr>
              {["Agent", "Calls Made", "Deals Closed", "Revenue", "Tasks Done", "Target", "Attainment"].map(h => (
                <th key={h} style={s.th}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {agents.map(agent => {
              const perf = performance[agent.id] || {};
              const attainment = Math.round((perf.deals / agent.target) * 100);
              return (
                <tr key={agent.id}>
                  <td style={s.td}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={s.avatar(30)}>{agent.avatar}</div>
                      <div>
                  ...
                  <td style={s.td}>{perf.calls}</td>
                  <td style={s.td}>{perf.deals}</td>
                  <td style={s.td}><span style={{ color: "#10b981", fontWeight: 600 }}>₱{perf.revenue?.toLocaleString()}</span></td>
                  <td style={s.td}>{perf.tasks}</td>
                  <td style={s.td}>{agent.target}</td>
                  <td style={s.td}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div style={{ flex: 1, height: 6, background: "#1e2535", borderRadius: 99 }}>
                        <div style={{ height: "100%", width: `${Math.min(attainment, 100)}%`, background: attainment >= 100 ? "#10b981" : attainment >= 70 ? "#f59e0b" : "#ef4444", borderRadius: 99 }} />
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 700, color: attainment >= 100 ? "#10b981" : "#f59e0b" }}>{attainment}%</span>
                    </div>
                  </td>
                </tr>
              );
  ...
  const AutomationsView = () => {
    const rules = [
      { icon: "📧", name: "Follow-up Reminder", desc: "Auto-remind agents when a lead hasn't been contacted in 3 days", status: "Active" },
      { icon: "⚠️", name: "Late Attendance Alert", desc: "Notify admin when an agent clocks in after 08:15", status: "Active" },
      { icon: "🏆", name: "Deal Won Celebration", desc: "Post to team channel when a deal is closed", status: "Active" },
      { icon: "📊", name: "Weekly Performance Report", desc: "Send performance summary every Monday 8AM", status: "Active" },
      { icon: "🔁", name: "Lead Reassignment", desc: "Auto-reassign leads if agent is absent 2+ days", status: "Inactive" },
    ];
    return (
      <div>
        {rules.map((r, i) => (
          <div key={i} style={{ ...s.card, display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 12, color: "#4b5a78", marginTop: 3 }}>{r.desc}</div>
            </div>
    ...
    <div>
      <div style={{ ...s.card, display: "flex", alignItems: "center", gap: 20, marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 700, color: "#60a5fa", marginTop: 4 }}>{currentTime.toLocaleTimeString("en-PH")}</div>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          {!clockedIn ? (
            <button style={{ ...s.btn("primary"), background: "#10b981" }} onClick={handleClockIn}>🟢 Clock In</button>
          ) : (
            <button style={{ ...s.btn("danger") }} onClick={handleClockOut}>🔴 Clock Out</button>
          )}
        </div>
      </div>
      {clockedIn && clockTime && (
        <div style={{ ...s.card, background: "#0d2a1a", borderColor: "#10b981", marginBottom: 20 }}>
          <div style={{ fontSize: 13, color: "#10b981" }}>✅ You clocked in at <strong>{clockTime}</strong>. Have a great day!</div>
        </div>
      )}
      <div style={s.grid4}>
        {[
          { label: "My Deals", val: myPerf.deals || 0, icon: "🤝" },
          { label: "Revenue", val: `₱${(myPerf.revenue || 0).toLocaleString()}`, icon: "💰" },
          { label: "Calls", val: myPerf.calls || 0, icon: "📞" },
          { label: "My Leads", val: myLeads.length, icon: "🎯" },
        ].map((stat, i) => (
          <div key={i} style={s.statCard}>
          ...
          {[...myAttendance].reverse().map((r, i) => {
            ...
            return (
              <tr key={i}>
                ...
                <td style={s.td}>{hours > 0 ? `${hours.toFixed(1)}h` : "—"}</td>
              </tr>
            );
          })}
        </tbody>
  ...
  const AgentPerformance = () => {
    ...
    return (
      <div>
        <div style={s.grid4}>
          {[
            { label: "Deals Closed", val: myPerf.deals || 0, target: currentUser.target, icon: "🤝" },
            { label: "Revenue Generated", val: `₱${(myPerf.revenue || 0).toLocaleString()}`, icon: "💰" },
            { label: "Calls Made", val: myPerf.calls || 0, icon: "📞" },
            { label: "Tasks Done", val: myPerf.tasks || 0, icon: "✅" },
          ].map((stat, i) => (
            <div key={i} style={s.statCard}>
              <div style={{ fontSize: 22, marginBottom: 6 }}>{stat.icon}</div>
              <div style={s.statVal}>{stat.val}</div>
              <div style={s.statLabel}>{stat.label}</div>
              {stat.target && <div style={{ fontSize: 11, color: "#4b5a78", marginTop: 4 }}>Target: {stat.target}</div>}
            </div>
          ))}
        </div>
        <div style={s.card}>
  ...
  const FreedomBoard = () => {
    ...
    const handleComment = (postId) => {
      ...
      if (!text) return;
      setBoardPosts(prev => prev.map(p => p.id !== postId ? p : {
        ...p, comments: [...p.comments, { author: currentUser.name, avatar: currentUser.avatar, text, date: today }]
      }));
      setCommentText(prev => ({ ...prev, [postId]: "" }));
    };

    const handlePin = (postId) => {
      setBoardPosts(prev => prev.map(p => p.id !== postId ? p : { ...p, pinned: !p.pinned }));
    };
    ...
    const handleSubmitPost = () => {
      ...
      setBoardPosts(prev => [{
        id: Date.now(), author: currentUser.name, avatar: currentUser.avatar,
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 22, flexWrap: "wrap" }}>
          {categories.map(cat => (
            <button key={cat} onClick={() => setFilterCat(cat)} style={{
              ...
              background: filterCat === cat ? (catColors[cat] || "#3b82f6") : "#1e2535",
              color: filterCat === cat ? "#fff" : "#8899b4", transition: "all 0.15s"
            }}>{cat !== "All" ? catIcons[cat] + " " : ""}{cat}</button>
          ))}
          <div style={{ marginLeft: "auto", fontSize: 12, color: "#4b5a78", display: "flex", alignItems: "center" }}>{filtered.length} posts</div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {filtered.map(post => {
            ...
            const totalReactions = Object.values(post.reactions).reduce((s, r) => s + r.length, 0);
            const canDelete = isAdmin || post.author === currentUser.name;
            return (
              <div key={post.id} style={{ background: "#161b27", borderRadius: 16, border: `1px solid ${post.pinned ? "#3b82f655" : "#1e2535"}`, overflow: "hidden", transition: "border-color 0.2s" }}>
                {/* Pin banner */}
                {post.pinned && (
                  <div style={{ background: "linear-gradient(90deg,#1e3a5f,#162032)", padding: "6px 20px", display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ fontSize: 12 }}>📌</span>
                    <span style={{ fontSize: 11, fontWeight: 700, color: "#60a5fa", letterSpacing: 0.5 }}>PINNED BY ADMIN</span>
                  </div>
                )}
                ...
                <div style={{ padding: "20px 22px" }}>
                  ...
                  <div style={{ display: "flex", gap: 12, marginBottom: 14, alignItems: "flex-start" }}>
                    <div style={{ width: 40, height: 40, borderRadius: "50%", background: "linear-gradient(135deg,#3b82f6,#8b5cf6)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: "#fff", flexShrink: 0 }}>{post.avatar}</div>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        <span style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>{post.author}</span>
                        <span style={{ display: "inline-block", padding: "2px 9px", borderRadius: 20, fontSize: 10, fontWeight: 700, background: (catColors[post.category] || "#4b5a78") + "25", color: catColors[post.category] || "#4b5a78" }}>{catIcons[post.category]} {post.category}</span>
                    ...
                    <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                      {isAdmin && (
                        <button onClick={() => handlePin(post.id)} title={post.pinned ? "Unpin" : "Pin"} style={{ background: "transparent", border: "none", cursor: "pointer", fontSize: 14, color: post.pinned ? "#60a5fa" : "#4b5a78", padding: 4 }}>📌</button>
                      )}
                      {canDelete && (
                        <button onClick={() => handleDelete(post.id)} title="Delete" style={{ background: "transparent", border: "none", cursor: "pointer", fontSize: 14, color: "#4b5a78", padding: 4 }}>🗑️</button>
                      )}
                    </div>
                  </div>
                  ...
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    {["👍", "❤️", "🔥"].map(emoji => {
                      ...
                      return (
                        <button key={emoji} onClick={() => handleReact(post.id, emoji)} style={{
                          display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 20, border: `1px solid ${reacted ? "#3b82f6" : "#1e2535"}`,
                          background: reacted ? "#1e3a5f" : "#0f1117", cursor: "pointer", fontSize: 13, color: reacted ? "#60a5fa" : "#8899b4", transition: "all 0.15s"
                        }}>
                          {emoji} {count > 0 && <span style={{ fontSize: 12, fontWeight: 600 }}>{count}</span>}
                        </button>
                      );
                    })}
                    <button onClick={() => setExpandedPost(isExpanded ? null : post.id)} style={{ marginLeft: "auto", background: "transparent", border: "none", cursor: "pointer", fontSize: 12, color: "#4b5a78", display: "flex", alignItems: "center", gap: 5 }}>
                  ...
                  {isExpanded && (
                    <div style={{ marginTop: 16, borderTop: "1px solid #1e2535", paddingTop: 16 }}>
                      {post.comments.length === 0 && <div style={{ fontSize: 12, color: "#4b5a78", marginBottom: 14 }}>No comments yet. Be the first!</div>}
          ...
          <div style={{ position: "fixed", inset: 0, background: "#000b", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200 }} onClick={() => setNewPostOpen(false)}>
            <div style={{ background: "#161b27", borderRadius: 20, border: "1px solid #1e2535", padding: 32, width: 520 }} onClick={e => e.stopPropagation()}>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {["Idea", "Question", "Concern", "Shoutout", "General"].map(cat => (
                    <button key={cat} onClick={() => setNewPost(p => ({ ...p, category: cat }))} style={{
                      padding: "7px 14px", borderRadius: 20, border: "none", cursor: "pointer", fontSize: 12, fontWeight: 600,
                      background: newPost.category === cat ? (catColors[cat] || "#3b82f6") : "#1e2535",
                      color: newPost.category === cat ? "#fff" : "#8899b4"
                    }}>{catIcons[cat]} {cat}</button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <textarea style={{ ...s.input, minHeight: 100, resize: "vertical" }} placeholder="Share your thoughts, ideas, or questions with the team…" value={newPost.body} onChange={e => setNewPost(p => ({ ...p, body: e.target.value }))} />
              </div>
  ...
  const renderContent = () => {
    if (isAdmin) {
      switch (activeTab) {
        ...
        case "overview": return <AgentOverview />;
        case "leads": return <LeadsView />;
        case "agents": return <AgentsView />;
        case "attendance": return <AttendanceView />;
        case "performance": return <PerformanceView />;
        case "automations": return <AutomationsView />;
        case "board": return <FreedomBoard />;
      ...
      switch (activeTab) {
        ...
        case "my-attendance": return <AgentAttendance />;
        case "my-performance": return <AgentPerformance />;
        case "board": return <FreedomBoard />;
        ...
        <div style={s.nav}>
          {tabs.map(tab => (
            <div key={tab} style={s.navItem(activeTab === tab)} onClick={() => setActiveTab(tab)}>
              <span style={s.navIcon}>{tabIcons[tab]}</span>
              {tabLabels[tab]}
            </div>
          ))}
        </div>
      ...
      <div style={s.main}>
        <div style={s.topbar}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
            <div style={{ fontSize: 13, color: "#4b5a78" }}>{currentTime.toLocaleTimeString("en-PH")}</div>
            <div style={{ position: "relative" }}>
              <button style={{ ...s.btn("ghost"), padding: "8px 12px", position: "relative" }} onClick={() => setShowNotif(!showNotif)}>
              ...
              {showNotif && (
                <div style={{ position: "absolute", right: 0, top: 44, width: 280, background: "#161b27", border: "1px solid #1e2535", borderRadius: 12, padding: 12, zIndex: 50 }}>
                  {notifications.map(n => (
                    <div key={n.id} style={{ padding: "8px 0", borderBottom: "1px solid #0f1117", fontSize: 12, color: "#c8d4e8" }}>
                      {n.type === "warning" ? "⚠️" : n.type === "success" ? "✅" : "ℹ️"} {n.text}
                    </div>
                  ))}
                </div>
              )}
      ...
      {leadModal && (
        <div style={s.modal} onClick={() => setLeadModal(null)}>
          <div style={s.modalBox} onClick={e => e.stopPropagation()}>
            ...
            {[["Contact", leadModal.contact], ["Email", leadModal.email], ["Phone", leadModal.phone], ["Status", leadModal.status], ["Value", `₱${leadModal.value.toLocaleString()}`], ["Assigned To", agents.find(a => a.id === leadModal.assignedTo)?.name], ["Date Added", leadModal.date], ["Notes", leadModal.notes]].map(([k, v]) => (
              <div key={k} style={{ marginBottom: 12 }}>
                <div style={s.label}>{k}</div>
                <div style={{ fontSize: 13, color: "#c8d4e8" }}>{v}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={s.modalBox} onClick={e => e.stopPropagation()}>
            ...
            {[["Company Name", "name", "text"], ["Contact Person", "contact", "text"], ["Email", "email", "email"], ["Phone", "phone", "text"], ["Value (₱)", "value", "number"], ["Notes", "notes", "text"]].map(([label, field, type]) => (
              <div key={field} style={{ marginBottom: 14 }}>
                ...
                <input style={s.input} type={type} value={newLead[field]} onChange={e => setNewLead(p => ({ ...p, [field]: e.target.value }))} />
              </div>
            ))}
            <div style={{ marginBottom: 14 }}>
              <label style={s.label}>Assign To</label>
              <select style={s.input} value={newLead.assignedTo} onChange={e => setNewLead(p => ({ ...p, assignedTo: parseInt(e.target.value) }))}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              ...
              <button style={s.btn("primary")} onClick={() => {
                setLeads(prev => [...prev, { ...newLead, id: Date.now(), status: "New", date: today, value: parseInt(newLead.value) || 0, assignedTo: newLead.assignedTo || agents[0]?.id }]);
                setAddLeadOpen(false);
                setNewLead({ name: "", contact: "", email: "", phone: "", value: "", notes: "" });
              }}>Add Lead</button>
            </div>
          </div>
        </div>
        ...
        <div style={s.modal} onClick={() => setAddAgentOpen(false)}>
          <div style={s.modalBox} onClick={e => e.stopPropagation()}>
            ...
            {[["Full Name", "name"], ["Email", "email"]].map(([label, field]) => (
              <div key={field} style={{ marginBottom: 14 }}>
                <label style={s.label}>{label}</label>
                <input style={s.input} value={newAgent[field]} onChange={e => setNewAgent(p => ({ ...p, [field]: e.target.value }))} />
              </div>
            ))}
            <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              ...
              <button style={s.btn("primary")} onClick={() => {
                const initials = newAgent.name.split(" ").map(n => n[0]).join("").toUpperCase().slice(0, 2);
                setAgents(prev => [...prev, { ...newAgent, id: Date.now(), role: "agent", avatar: initials, status: "active" }]);
                setAddAgentOpen(false);
                setNewAgent({ name: "", email: "", department: "Sales", target: 40 });
              }}>Add Agent</button>
            </div>
          </div>
        </div>
